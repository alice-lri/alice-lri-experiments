from scripts.common.helper.entities import (
    ...
    IntrinsicsExperiment,
    LocalGeometryExperiment,
    RangeImageExperiment,
)
...
def fetch_local_geometry_experiment_id(db_path: str) -> int:
    with Database(db_path) as db:
        experiments = LocalGeometryExperiment.all(db)
        assert len(experiments) == 1, f"Expected exactly one experiment, got {len(experiments)}"
        return experiments[0].id
