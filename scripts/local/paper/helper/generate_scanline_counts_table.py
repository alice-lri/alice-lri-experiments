...
def main():
    print(f"Loading data from database at {Config.DB_PATH}")
    experiment_id = fetch_experiment_id()
...
def fetch_experiment_id() -> int:
    query = """
        SELECT id FROM experiment
        WHERE use_hough_continuity == 1 AND use_scanline_conflict_solver == 1
          AND use_vertical_heuristics == 1  AND use_horizontal_heuristics
    """

    df = pd_read_sqlite_query(Config.DB_PATH, query)
